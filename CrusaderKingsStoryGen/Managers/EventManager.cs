// <copyright file="EventManager.cs" company="Yemmlie - 252afh fork">
...
namespace CrusaderKingsStoryGen.Managers
{
    ...
    class EventManager
    {
        ...
        public void AddToNamespace(string names, EventParser ev)
        {
            ...
            if (!this.NamespaceMap.ContainsKey(names))
            {
                e = new EventNamespace(names);
                this.NamespaceMap[names] = e;
            }
            else
            {
                e = this.NamespaceMap[names];
            }
        ...
        public void Load()
        {
            ...
            foreach (var file in files)
            {
                ...
                for (int index = 0; index < s.Root.Children.Count; index++)
                {
                    ...
                    this.EventMap[(sc.Find("id") as ScriptCommand).Value.ToString()] = ee;
                    if (names != null)
                    {
                        this.AddToNamespace(names, ee);
                    }
                }
        ...
        public void Save()
        {
            ...
            for (int index = 0; index < this.Scripts.Count; index++)
            {
                ...
                var toRemove = this.ConvertReligionTests(script.Root, toDelete);
                if (toRemove != null)
                {
                ...
                if (child is ScriptScope)
                    {
                        ...
                        this.ConvertReligionTests(child as ScriptScope, toDelete);
        ...
        {
            foreach (var str in this.ExcludeFile)
            {
        ...
        private bool TestRemove(ScriptScope scriptScope)
        {
            ...
            if (c != null)
            {
                ...
                else if (c.Value is int)
                {
                    ...
                    if (val2 != null && this.ExcludeID.Contains(val2))
                    {
                        return true;
                    }
                }
