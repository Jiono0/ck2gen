// <copyright file="TraitManager.cs" company="Yemmlie - 252afh fork">
...
namespace CrusaderKingsStoryGen.Managers
{
    ...
    class TraitManager
    {
        ...
        potential = {
           ...
           secretly_" + religion.Name + @" = {
            ...
            this.opposites.Add(op);
             this.AddTrait("secretly_" + relName, "bastard.dds");
            this.AddTrait("sympathy_" + relName, "bastard.dds");
        }

        public void Save()
        {
            return;
            foreach (var scriptScope in this.opposites)
            {
                string name = scriptScope.Parent.Name;
                foreach (var opposite in this.opposites)
                {
        ...
        public void FillCharacter(CharacterParser characterParser, CharacterParser.PrimaryTrait trait)
        {
            ...
            var choice = eduChoices[RandomIntHelper.Next(eduChoices.Count)];
            characterParser.Scope.Add(new ScriptCommand("trait", choice.Name, characterParser.Scope));

            exclude.Add("twin");
            int numTraits = RandomIntHelper.Next(3, 8);

            for (int x = 0; x < numTraits; x++)
            {
                var ch = choicesL[RandomIntHelper.Next(choices.Count)];
