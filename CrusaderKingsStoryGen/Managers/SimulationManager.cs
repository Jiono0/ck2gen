// <copyright file="SimulationManager.cs" company="Yemmlie - 252afh fork">
...
namespace CrusaderKingsStoryGen.Managers
{
    ...
    class SimulationManager
    {
        ...
        public void Init()
        {
            ...
            s.Name = StarHelper.SafeName(name);
        ...
        {
            CharacterParser chr = CharacterManager.instance.CreateNewCharacter(culture, religion, RandomIntHelper.Next(8) == 0);
        ...
        private static void FixupDuchies()
        {
            ...
            {
                var titleParser = orphans[index];
                int kingSize = RandomIntHelper.Next(20 * 20);
        ...
        private void CreateKingdomsFromDuchies()
        {
            ...
            for (int index = 0; index < duchies.Count; index++)
            {
                ...
                int kingSize = RandomIntHelper.Next(15, 24);
        ...
        private void CreateEmpiresFromKingdoms()
        {
            ...
            var empire = empires[RandomIntHelper.Next(empires.Count)];
            {
        ...
        {
            if (!this.bDonePreStage2 && MapManager.instance.ToFill.Count == 0)
            {
                this.bDonePreStage1 = true;
            }
            ...
            this.bTicked = true;
            if (this.bDonePreStage1)
            {
                this.AutoValidateRealm = false;
                {
                    CharacterParser[] a = this.characters.ToArray();
                ...
                try
                {
                    this.CreateKingdomsFromDuchies();
                    this.CreateEmpiresFromKingdoms();
                    this.AutoValidateRealm = true;
                ...
                this.bDonePreStage1 = false;
                this.bDonePreStage2 = true;
            }

            if (this.bDonePreStage2)
            {
        ...
        private void DoPrehistoryTick()
        {
            ...
            for (int n = 0; n < 100; n++)
            {
                ...
                {
                    CharacterParser character = this.characters[x];
                    ...
                    if (RandomIntHelper.Next((int)chanceOfRevolt) == 0)
                    {
                        this.HandleRevolt(character);
                ...
                if (numTicking == 0)
                {
                    for (int x = (int)(this.characters.Count / 1.5f); x < this.characters.Count; x++)
                    {
                        CharacterParser character = this.characters[this.characters.Count - 1 - x];
            ...
            if (toFillCount == MapManager.instance.ToFill.Count && toFillCount > 0)
            {
                ...
                for (int index = 0; index < MapManager.instance.ToFill.Count; index++)
                {
                    ...
                    if (dist < smallestDist && provinceParser.ProvinceTitle == null)
                    {
        ...
        {
            this.NumberOfIndependentCounts = 0;
            this.NumberOfIndependentDukes = 0;
            this.NumberOfIndependentKings = 0;
            this.NumberOfIndependentEmpire = 0;
...
/*
          ...
          */
            ...
            {
                if (this.DestroyedTitles.Contains(titleParser))
                {
                    if (titleParser.Holder != null)
                    {
                        titleParser.Holder = null;
                ...
                if (titleParser.Holder != null)
                {
                    ...
                    if (titleParser.Holder.IsAlive)
                    {
                        ...
                        this.SimulationCountRank(titleParser);
                      ...
                      //  CorrectTopmostLiegeMismatch(chr);
                        ...
                        {
                            if (TestAssimilationToBigNeighbour(titleParser, chr))
                            {
                                continue;
                            }
                        }
                     ...
                     //   DoTech(chr);
                        ...
                        this.DoChildrenSimulation(chr, titleParser);
                    }
            ...
            this.LastNumberOfIndependentCounts = this.NumberOfIndependentCounts;
            this.LastNumberOfIndependentDukes = this.NumberOfIndependentDukes;
            this.LastNumberOfIndependentKings = this.NumberOfIndependentKings;
            this.LastNumberOfIndependentEmpire = this.NumberOfIndependentEmpire;
        }
        ...
        private void CalculateConquerersAndCollapsers(TitleParser[] arr, int useIdealEmpire, List<TitleParser> potentialCollapsers)
        {
            foreach (var titleParser in arr)
            {
                ...
                if (titleParser.Rank == 4)
                {
                    ...
                    switch (MutationHelper.EmpireStability)
                    {
                        case 5:
                            chanceOfCollapse = 0;
                            break;
                    ...
                    {
                        this.MajorCollapse(titleParser);
                    }
                }

                if (useIdealEmpire < this.LastNumberOfIndependentEmpire)
                {
                    if (titleParser.Holder != null)
                    {
                        if (titleParser.Liege == null && titleParser.Rank == 4)
                        {
                            if (RandomIntHelper.Next(5) == 0 && !titleParser.Holder.IsMajorPlayer)
                            {
                ...
                if (useIdealEmpire >= this.LastNumberOfIndependentEmpire &&
                    MutationHelper.IdealIndependentKingCount > this.LastNumberOfIndependentKings)
                {
                    if (titleParser.Holder != null)
                    {
        ...
        private static void TriggerConquerers(List<TitleParser> potentialConquerers)
        {
            ...
            {
                var item = potentialConquerers[RandomIntHelper.Next(potentialConquerers.Count)];
        ...
        private void CollapseTitles(List<TitleParser> potentialCollapsers)
        {
            ...
            {
                var item = potentialCollapsers[RandomIntHelper.Next(potentialCollapsers.Count)];
                var chr = item.Holder;

                if (item.Holder != null && RandomIntHelper.Next(180) == 0)
                {
                    ...
                    if (item.Rank == 4 && !item.Holder.IsMajorPlayer)
                    {
                        ...
                        this.MajorCollapse(title); //title.SoftDestroy();
                    }
                    else
                    {
                        if (item.Rank == 3 && this.LastNumberOfIndependentDukes < MutationHelper.IdealIndependentDukeCount &&
                            !item.Holder.IsMajorPlayer)
                        {
                            var title = chr.PrimaryTitle;

                            this.MajorCollapse(title);
                        }
                        else
                        {
        ...
        private static void RepairBrokenTitles(TitleParser titleParser)
        {
            if (titleParser.government == "republic")
            {
                ...
                else
                {
                    ...
                    if (!found)
                    {
                        var chosen = choices[RandomIntHelper.Next(choices.Count)];
        ...
        private void SimulationCountRank(TitleParser titleParser)
        {
            if (titleParser.Liege == null)
            {
                if (titleParser.Rank == 1)
                {
                    this.NumberOfIndependentCounts++;
                }

                if (titleParser.Rank == 2)
                {
                    this.NumberOfIndependentDukes++;
                }

                if (titleParser.Rank == 3)
                {
                    this.NumberOfIndependentKings++;
                }

                if (titleParser.Rank == 4)
                {
                    this.NumberOfIndependentEmpire++;
                }
            }
        }
        ...
        //    return false;
            ...
            if (titleParser.Rank <= 1 && titleParser.Liege == null && titleParser.Holder != null && titleParser.Holder.TopLiegeCharacter.PrimaryTitle.Rank <= 2)
            {
                ...
                if (biggiesAround.Count == 1)
                {
                    if (RandomIntHelper.Next(20) == 0)
                    {
                        var b = biggiesAround[RandomIntHelper.Next(biggiesAround.Count)];
        ...
        private static void TestPropogateReligion(CharacterParser chr)
        {
            if (!ReligionManager.instance.ReligionMap.ContainsKey(chr.religion))
            {
                ...
                if (choices.Count > 0)
                {
                    chr.religion = choices[RandomIntHelper.Next(choices.Count)];
                }
                else
                {
                    chr.religion =
                        ReligionManager.instance.AllReligions[
                            RandomIntHelper.Next(ReligionManager.instance.AllReligions.Count)].Name;
                }
            }
        ...
        private static void DoCreateDejureTitle(CharacterParser chr, TitleParser titleParser, List<TitleParser> titles)
        {
            ...
            if (percent > am && bShouldMakeTitle && RandomIntHelper.Next(50) == 0)
            {
...
// check if dies
            ...
            if (chr.Age >= 16)
            {
                ...
                if (hasKid)
                {
                    CharacterParser kid = CharacterManager.instance.CreateNewCharacter(chr.Dynasty,
                        RandomIntHelper.Next(2) == 0, SimulationManager.instance.Year, titleParser.CapitalProvince.Religion.Name,
                        titleParser.CapitalProvince.Culture.Name);
                    ...
                    if (RandomIntHelper.Next(2) == 0)
                    {
                        ...
                        if (choices.Count > 4)
                        {
                            name = choices[RandomIntHelper.Next(choices.Count)];
                            kid.ChrName = null;
                        }
                    }
        ...
        private void MinorWar()
        {
            ...
            if (list.Any())
            {
                var toWar = list[RandomIntHelper.Next(list.Count)];
                ...
                toWar.Wars.Add(choices[RandomIntHelper.Next(choices.Count)]);
        ...
        void MajorCollapse(TitleParser toDestroy = null)
        {
            ...
            if (toDestroy == null)
            {
                ...
                if (list.Any())
                {
                    toDestroy = list[RandomIntHelper.Next(Math.Min(3, list.Count))];
                }
                else
                {
                    return;
            ...
            {
                {
                    ...
                    if (toDestroy.Rank == 4 && RandomIntHelper.Next(7) == 0)
                    {
        ...
        void MajorConquerer()
        {
            ...
            if (list.Any())
            {
                TitleParser title = list[RandomIntHelper.Next(list.Count)];
        ...
        {
            if (MainForm.instance.SelectedTitleLast == breakTest)
            {
                Debugger.Break();
                MainForm.instance.SelectedTitleLast = null;
            }
        }
        ...
        private void HandleTitles(CharacterParser chr)
        {
            ...
            while (chr.NumberofKingTitles >= 3 && chr.PrimaryTitle.Rank > 3 && !chr.IsMajorPlayer)
            {
                ...
                var giveAway = choices[RandomIntHelper.Next(choices.Count)];
            ...
            while (chr.NumberofCountTitles > 5 && chr.PrimaryTitle.Rank > 1)
            {
                ...
                {
                    var giveAway = choiceProvinces[RandomIntHelper.Next(choices.Count)];
            ...
            for (var index = 0; index < chr.Titles.Count; index++)
            {
                ...
                if (titleParser.Liege == null)
                {
                    ...
                    this.SortVassals(titleParser, titleParser.Holder);
                }
            }
        }
        ...
        private void SortVassals(TitleParser title, CharacterParser holder)
        {
            ...
            foreach (var titleParser in title.DejureSub)
            {
               ...
               //         titleParser.Value.DoSetLiegeEvent(title);
                    ...
                    this.SortVassals(titleParser.Value, holder);
                }
            }
        }
        ...
        private void HandleRevolt(CharacterParser character)
        {
            ...
            foreach (var title in character.Titles)
            {
                ...
                if (title.Owns.Count > 0)
                {
                    ...
                    {
                        if (provinceParser.land && (provinceParser.ProvinceTitle == null || provinceParser.Title.Holder == null))
                        {
                            bDo = true;
                            break;
                        }
            ...
            var p = pr[RandomIntHelper.Next(pr.Count)];
            ...
            foreach (var provinceParser in provinces)
            {
                if (provinceParser.ProvinceTitle != null)
                {
                    ch.GiveTitle(TitleManager.instance.TitleMap[provinceParser.ProvinceTitle]);
                }
            }
            ...
            if (RandomIntHelper.Next(Globals.OneInChanceOfCultureSplinter) == 0)
            {
        ...
        {
           // Rand.SetSeed();
            this.AllowCapitalPicking = false;
            this.bDonePreStage2 = true;
            this.PreYear = 1;
            this.Year = endDate;
            this.AutoValidateRealm = false;
            ...
            foreach (var title in TitleManager.instance.Titles)
            {
                if (title.Rank == 4)
                {
                    if (title.DejureSub.Count > 0)
                    {
                        ...
                        this.PlaceEmpireRandomlyOnMap(title);
                    }
                }
            }
